# RFC-0002: Runtime-Contract-v0.4 Promotion Scope
...
`runtime-contract-v0.3` established execution-mode metadata and cross-repo promotion discipline.
The next baseline (`runtime-contract-v0.4`) should harden those guarantees into explicit contract obligations that downstream repos can validate automatically.
...
3. Record v0.4 evidence links in:
...
1. `t81-vm` contract artifact has `contract_version` bumped to the v0.4 value.
...
2. Which vector set is mandatory for v0.4 parity proof (`arithmetic`, `faults`, tensor slice)?
